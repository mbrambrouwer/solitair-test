...
public class CardMoveChecks {
    ...
    public static void deckLevelChecks(Deck sourceDeck, int sourceCardIndex, Deck destinationDeck) throws MoveException {
        // TODO: Write implementation
        System.out.println(sourceDeck.size());
    ...
    public static void cardLevelChecks(Deck targetDeck, Card cardToAdd) throws MoveException {
        ...
        if (targetDeck.getDeckType() == DeckType.STACK) {
            if (targetDeck.get(targetDeck.size()).getSuit() == cardToAdd.getSuit()) {
                System.out.println("test");
            }
        }

        System.out.println(targetDeck.getDeckType());
        System.out.println(cardToAdd.getRank().ordinal());
        if (targetDeck.getDeckType() == DeckType.COLUMN) {
//            if (targetDeck.get(targetDeck.size()).getRank() == Rank.ACE && cardToAdd.getRank() == Rank.TWO) {
//                return;
//            }
            if (targetDeck.get(targetDeck.size()).getRank().ordinal() != cardToAdd.getRank().ordinal() - 1) {
                throw new MoveException("error");
            }
        }
        if (targetDeck.getDeckType() == DeckType.STACK) {
            if (targetDeck.get(targetDeck.size()).getRank().ordinal() - 1 != cardToAdd.getRank().ordinal()) {
                throw new MoveException("error");
            }
    ...
    static boolean opposingColor(Card card1, Card card2) {
        // TODO: Write implementation
        if (card1.getSuit() == Suit.DIAMONDS || card1.getSuit() == Suit.HEARTS) {
            ...
            if (card2.getSuit() == Suit.DIAMONDS || card2.getSuit() == Suit.HEARTS) {
                return true;
            } else {
                return false;
            }
